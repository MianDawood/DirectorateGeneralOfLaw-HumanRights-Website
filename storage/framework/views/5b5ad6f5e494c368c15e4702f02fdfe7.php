...
unset($__errorArgs, $__bag); ?>
    ...
    <div>
        ...
        <input type="datetime-local" name="event_date" id="event_date"
               value="<?php echo e(old('event_date', $event?->event_date ? $event->event_date->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i'))); ?>"
               class="mt-1 block w-full px-3 py-2 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
               required>
...
unset($__errorArgs, $__bag); ?>
    ...
    <div>
        ...
        <input type="number" name="order" id="order" value="<?php echo e(old('order', $event?->order ?? 0)); ?>" min="0"
               class="mt-1 block w-full px-3 py-2 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
...
unset($__errorArgs, $__bag); ?>
    ...
    <div class="flex flex-col gap-3 justify-end">
        ...
        <label class="inline-flex items-center">
            <input type="checkbox" name="is_active" value="1" <?php echo e(old('is_active', $event?->is_active ?? true) ? 'checked' : ''); ?>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-4">
            <?php $__currentLoopData = $event->images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <label class="relative block rounded-lg overflow-hidden border border-gray-200 cursor-pointer group">
                    <img src="<?php echo e(asset('storage/' . $image->image_path)); ?>" alt="Event image" class="w-full h-28 object-cover">
...
<div class="mt-8 pt-6 border-t border-gray-200">
    ...
    <?php if($event && $event->videos->isNotEmpty()): ?>
        <div class="space-y-3 mb-4">
            <?php $__currentLoopData = $event->videos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $video): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <img src="https://img.youtube.com/vi/<?php echo e($video->youtube_video_id); ?>/default.jpg" alt="" class="w-20 h-12 object-cover rounded">
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-gray-800 truncate"><?php echo e($video->youtube_url); ?></p>
                    </div>
                    <label class="inline-flex items-center text-sm text-red-600 shrink-0">
                        <input type="checkbox" name="remove_videos[]" value="<?php echo e($video->id); ?>" class="rounded border-gray-300 text-red-600 mr-1">
                        Remove
                    </label>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
function manageSubjects() {
    return {
        ...
        async fetchAll() {
            try {
                const res = await fetch('<?php echo e(route('admin.event-categories.index')); ?>', {
        ...
        async add() {
            ...
            this.error = '';
            try {
                const res = await fetch('<?php echo e(route('admin.event-categories.store')); ?>', {
                    method: 'POST',
                    headers: {
        ...
        async save(cat) {
            ...
            try {
                const res = await fetch(
                    '<?php echo e(route('admin.event-categories.update', ['category' => '__ID__'])); ?>'.replace('__ID__', cat.id),
                    {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
        ...
        async remove(cat) {
            ...
            try {
                const res = await fetch(
                    '<?php echo e(route('admin.event-categories.destroy', ['category' => '__ID__'])); ?>'.replace('__ID__', cat.id),
                    {
                        method: 'DELETE',
                        headers: {
                            'Accept': 'application/json',
...
<?php $__env->stopPush(); ?>
<?php /**PATH /Applications/MAMP/htdocs/human-rights-kp/resources/views/pages/dashboard/events/_form.blade.php ENDPATH**/ ?>