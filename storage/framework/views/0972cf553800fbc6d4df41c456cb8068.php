...
<?php $component->withAttributes([]); ?>
    ...
    <div class="relative bg-[#123B2D] py-16 lg:py-24 overflow-hidden">
        <div class="absolute inset-0 opacity-10 bg-[url('<?php echo e(asset('images/hero image 1.jpg')); ?>')] bg-cover bg-center mix-blend-overlay"></div>
    ...
    <section class="bg-slate-50 py-16">
        <div class="max-w-4xl mx-auto px-4 sm:px-6">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 md:p-12">
                ...
                <form class="space-y-6" method="POST" action="<?php echo e(route('complaints.store')); ?>" enctype="multipart/form-data">
                    <?php echo csrf_field(); ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Full Name *</label>
                            <input type="text" name="full_name" value="<?php echo e(old('full_name')); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-[#02B1EB] focus:ring-1 focus:ring-[#02B1EB] outline-none transition-all" placeholder="Enter your full name" required>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">CNIC Number *</label>
                            <input type="text" name="cnic" value="<?php echo e(old('cnic')); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-[#02B1EB] focus:ring-1 focus:ring-[#02B1EB] outline-none transition-all" placeholder="00000-0000000-0" required>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Contact Number *</label>
                            <input type="tel" name="contact_number" value="<?php echo e(old('contact_number')); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-[#02B1EB] focus:ring-1 focus:ring-[#02B1EB] outline-none transition-all" placeholder="03XX-XXXXXXX" required>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">District</label>
                             <input type="text" name="district" value="<?php echo e(old('district')); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-[#02B1EB] focus:ring-1 focus:ring-[#02B1EB] outline-none transition-all" placeholder="Enter district name">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="category" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-[#02B1EB] focus:ring-1 focus:ring-[#02B1EB] outline-none transition-all text-slate-500 rounded-lg">
                            <option <?php echo e(old('category') === 'Property Dispute Rights' ? 'selected' : ''); ?>>Property Dispute Rights</option>
                            <option <?php echo e(old('category') === 'Child Labor / Protection' ? 'selected' : ''); ?>>Child Labor / Protection</option>
                            <option <?php echo e(old('category') === 'Women Harassment' ? 'selected' : ''); ?>>Women Harassment</option>
                            <option <?php echo e(old('category') === 'Violence / Police Brutality' ? 'selected' : ''); ?>>Violence / Police Brutality</option>
                            <option <?php echo e(old('category') === 'Other Minorities Issues' ? 'selected' : ''); ?>>Other Minorities Issues</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Complaint Details *</label>
                        <textarea rows="5" name="details" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-[#02B1EB] focus:ring-1 focus:ring-[#02B1EB] outline-none transition-all resize-none" placeholder="Provide a detailed account of the incident..." required><?php echo e(old('details')); ?></textarea>
                    </div>
...
<?php endif; ?>
<?php /**PATH /Applications/MAMP/htdocs/human-rights-kp/resources/views/pages/complaint_cell.blade.php ENDPATH**/ ?>