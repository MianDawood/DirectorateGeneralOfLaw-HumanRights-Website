...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load calendar only if calendar exists
    if (document.querySelector('#calendar')) {
        import('@fullcalendar/core').then(module => {
            window.FullCalendar = module.Calendar;
            import('./components/calendar-init').then(module => module.calendarInit());
        });
    }

    // Load flatpickr only if date inputs exist
    if (document.querySelector('.flatpickr-input') || document.querySelector('[data-datepicker]')) {
