        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.page-contents.store') }}" enctype="multipart/form-data">
                        @csrf
                            ...
                            <div>
                                ...
                                <select name="page_key" class="w-full border-gray-300 rounded-md shadow-sm focus:border-brand-500 focus:ring-brand-500">
                                    @foreach($pages as $key => $name)
                                        <option value="{{ $key }}" {{ $pageKey === $key ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="flex justify-end mt-6 gap-4">
                            <a href="{{ route('admin.page-contents.index', ['page' => $pageKey]) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Cancel</a>
