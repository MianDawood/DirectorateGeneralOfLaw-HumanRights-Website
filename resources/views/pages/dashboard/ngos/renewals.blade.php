        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                    ...
                    <form method="GET" action="{{ route('admin.ngos.renewals.index') }}" class="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        <div>
                            <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Scope</label>
                            <select name="scope"
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                <option value="due" @selected(request('scope', $scope) === 'due')>Due / Expiring (90 days)</option>
                                <option value="all" @selected(request('scope', $scope) === 'all')>All Approved</option>
                            </select>
                        </div>
                        <div>
                        ...
                        <div class="flex gap-2 items-end">
                            ...
                            <a href="{{ route('admin.ngos.renewals.index') }}"
                                ...
                                @forelse($ngos as $ngo)
                                    ...
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                            {{ $ngo->profile?->organization_name ?: 'N/A' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $ngo->profile?->district ?: '—' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-mono">{{ $ngo->registration_no ?: '—' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ optional($ngo->certificate_issue_date)->format('Y-m-d') ?: '—' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            @if($isExpired)
                                                <span class="text-red-600 font-semibold">{{ optional($ngo->expiry_date)->format('Y-m-d') }}</span>
                                            @elseif($urgent)
                                                <span class="text-amber-600 font-semibold">{{ optional($ngo->expiry_date)->format('Y-m-d') }}</span>
                                            @else
                                                {{ optional($ngo->expiry_date)->format('Y-m-d') ?: '—' }}
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $daysLeft === null ? '—' : ($isExpired ? abs($daysLeft) . ' days overdue' : $daysLeft . ' days') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $ngo->last_renewal_date ? 'Renewed ' . $ngo->last_renewal_date->format('Y-m-d') : 'Never' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium"
                                            x-data="renewModal({
                                                years: 3,
                                                ngoId: {{ $ngo->id }},
                                                ngoName: @js($ngo->profile?->organization_name ?: 'N/A'),
                                                regNo: @js($ngo->registration_no ?: '—'),
                                                currentExpiry: @js(optional($ngo->expiry_date)->format('d M Y') ?: '—'),
                                                expiryDate: @js($ngo->expiry_date ? $ngo->expiry_date->format('Y-m-d') : null),
                                                daysLeft: {{ $daysLeft === null ? 'null' : $daysLeft }},
                                                isExpired: {{ $isExpired ? 'true' : 'false' }},
                                                scope: @js(request('scope', $scope)),
                                                district: @js(request('district')),
                                                thematicArea: @js(request('thematic_area'))
                                            })">
                                            ...
                                            <form method="POST" id="renew-form-{{ $ngo->id }}"
                                                  action="{{ route('admin.ngos.renewals.renew', $ngo) }}">
                                                @csrf
                                                <input type="hidden" name="renew_years" :value="years">
                                                <input type="hidden" name="scope" value="{{ request('scope', $scope) }}">
                                                ...
                                                <div class="flex items-center justify-center min-h-screen px-4">
                                                    ...
                                                    <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md p-6 my-8">
                                                        ...
                                                        <div class="mt-6 flex justify-end gap-2">
                                                            ...
                                                            <button type="submit" form="renew-form-{{ $ngo->id }}"
