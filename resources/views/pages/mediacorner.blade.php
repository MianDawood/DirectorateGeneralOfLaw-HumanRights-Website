            ...
            <div class="max-w-[1536px] mx-auto px-6 lg:px-20">
                <div class="flex items-center gap-3 py-4 overflow-x-auto scrollbar-hide">
                    <a href="{{ route('mediacorner') }}"
                       class="shrink-0 px-5 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-full transition-all duration-300 {{ ($filter === 'all' || !$filter) ? 'bg-[#123B2D] text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                        All
                    </a>
                    <a href="{{ route('mediacorner', ['filter' => 'news']) }}"
                       class="shrink-0 px-5 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-full transition-all duration-300 {{ $filter === 'news' ? 'bg-[#123B2D] text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                        News
                    </a>
                    @foreach($subjects as $subject)
                        <a href="{{ route('mediacorner', ['filter' => $subject]) }}"
                           class="shrink-0 px-5 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-full transition-all duration-300 {{ $filter === $subject ? 'bg-[#123B2D] text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                            {{ $subject }}
                        </a>
                    @endforeach
                </div>
                            ...
                            <div class="group bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                                <div class="relative h-60 overflow-hidden">
                                    @if($item->item_type === 'news')
                                        <img src="{{ $item->image_path ? asset('storage/' . $item->image_path) : asset('images/event 6.jpg') }}"
                                             alt="{{ $item->title }}"
                                             class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                                    @else
                                        <img src="{{ $item->coverImageUrl() ?? asset('images/event 1.jpeg') }}"
                                             alt="{{ $item->title }}"
                                             class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                                    @endif
                                    <div class="absolute top-4 left-4">
                                        ...
                                        @elseif($item->subject)
                                            <span class="bg-[#123B2D] text-white text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg shadow-lg">{{ $item->subject }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="p-8">
                                    <div class="flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-widest mb-4">
                                        ...
                                        @if($item->item_type === 'news')
                                            {{ $item->published_date ? $item->published_date->format('M d, Y') : 'N/A' }}
                                        @else
                                            {{ $item->event_date ? $item->event_date->format('M d, Y') : 'N/A' }}
                                        @endif
                                    </div>
