...
<aside id="sidebar"
                ...
                @foreach ($menuGroup['items'] as $itemIndex => $item)
                    @if (isset($item['subItems']))
                        @foreach ($item['subItems'] as $subItem)
                            if (currentPath === '{{ ltrim(parse_url($subItem['path'], PHP_URL_PATH), '/') }}') {
    ...
    <div class="pt-8 pb-7 flex items-center gap-3"
        ...
        <a href="/" class="flex items-center gap-3">
            <img src="{{ $settings->logo ? asset('storage/' . $settings->logo) : asset('images/logo.jpg') }}" alt="Logo" class="w-10 rounded-full" />
                            ...
                            @foreach ($menuGroup['items'] as $itemIndex => $item)
                                <li>
                                    @if (isset($item['subItems']))
                                     ...
                                     @else
                                        <!-- Simple Menu Item -->
                                        @if (isset($item['is_logout']) && $item['is_logout'])
