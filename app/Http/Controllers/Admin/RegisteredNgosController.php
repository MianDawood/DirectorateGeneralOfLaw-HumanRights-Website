...
class RegisteredNgosController extends Controller
{
    use NgoFilters;
    ...
    public function index(Request $request)
    {
        ...
        return view('pages.dashboard.ngos.registered', [
            'ngos' => $ngos,
            'type' => 'registered',
    ...
    public function export(Request $request)
    {
        ...
        $prefix = 'registered_ngos_' . now()->format('Y-m-d');
        ...
        return Excel::download(new NgosExport($ngos, 'registered'), $prefix . '.xlsx');
    }
    ...
    private function applyFilters($query, Request $request): void
    {
        ...
        if ($from = $request->input('date_from')) {
            $query->whereDate('certificate_issue_date', '>=', $from);
        }
        if ($to = $request->input('date_to')) {
            $query->whereDate('certificate_issue_date', '<=', $to);
        }
    }
}
